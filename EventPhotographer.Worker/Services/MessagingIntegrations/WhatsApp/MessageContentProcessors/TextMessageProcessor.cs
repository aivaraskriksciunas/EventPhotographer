...
internal class TextMessageProcessor(
    ...
    ICommandHandler<JoinEvent, Participant> joinEventHandler) 
    : IMessageContentProcessor
{
    ...
    public async Task ProcessMessageContentAsync(WhatsAppMessage message, JsonElement json)
    {
        ...
        var actionResult = await joinEventHandler.HandleAsync(new JoinEvent
        {
            Code = code,
            Name = message.WhatsAppContact.ProfileName
        });
        ...
        message.WhatsAppContact.ActiveParticipant = actionResult.Value;
        await whatsAppContactService.UpdateAsync(message.WhatsAppContact);

        await whatsAppClient.ReplyToMessage(
            message,
            $"You have joined '{actionResult.Value.Event.Name}'! You can send me the pictures you want to share with the hosts at any time. I will react to them when I receive them :) Have fun!");
    }
